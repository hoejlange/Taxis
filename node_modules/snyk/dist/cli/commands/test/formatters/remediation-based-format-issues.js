...
function constructLicenseText(basicLicenseInfo, testOptions) {
    ...
    for (const id of Object.keys(basicLicenseInfo)) {
        const licenseText = formatIssue(id, basicLicenseInfo[id].title, basicLicenseInfo[id].severity, basicLicenseInfo[id].isNew, `${basicLicenseInfo[id].name}@${basicLicenseInfo[id].version}`, basicLicenseInfo[id].paths, testOptions, basicLicenseInfo[id].note, basicLicenseInfo[id].legalInstructions);
...
function constructPatchesText(patches, basicVulnInfo, testOptions) {
    ...
    for (const id of Object.keys(patches)) {
        ...
        const thisPatchFixes = formatIssue(id, basicVulnInfo[id].title, basicVulnInfo[id].severity, basicVulnInfo[id].isNew, `${basicVulnInfo[id].name}@${basicVulnInfo[id].version}`, basicVulnInfo[id].paths, testOptions, basicVulnInfo[id].note);
...
function thisUpgradeFixes(vulnIds, basicVulnInfo, testOptions) {
    return vulnIds
        ...
        .map((id) => formatIssue(id, basicVulnInfo[id].title, basicVulnInfo[id].severity, basicVulnInfo[id].isNew, `${basicVulnInfo[id].name}@${basicVulnInfo[id].version}`, basicVulnInfo[id].paths, testOptions, basicVulnInfo[id].note, [], basicVulnInfo[id].reachability))
        .join('\n');
}
...
function constructUnfixableText(unresolved, basicVulnInfo, testOptions) {
    ...
    for (const issue of unresolved) {
        ...
        unfixableIssuesTextArray.push(formatIssue(issue.id, issue.title, issue.severity, issue.isNew, `${issue.packageName}@${issue.version}`, issueInfo.paths, testOptions, issueInfo.note, [], issue.reachability) + `${extraInfo}`);
    }
...
function formatIssue(id, title, severity, isNew, vulnerableModule, paths, testOptions, note, legalInstructions, reachability) {
    ...
    return (severitiesColourMapping[severity].colorFunc(`  ✗ ${chalk_1.default.bold(title)}${newBadge} [${titleCaseText(severity)} Severity]`) +
        reachabilityText +
        `[${config.ROOT}/vuln/${id}]` +
        name +
        introducedBy +
